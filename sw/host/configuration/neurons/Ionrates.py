...
class Pospischil:
    V_T         = -30       # (mV) adjust spike threshold USED TO BE -55S
    V_X         = 2         # (mV)
    TAU_MAX     = 1e3       # (ms)
...
class Pyramidal:
    ...
    # Na ---------------------------------------------------------------------
    # m
    def alpha_m_Na(self, v) -> np.longdouble: return ((-0.32*(v-self.V_T-13)) / (exp(-(v-self.V_T-13)/4)-1)) # v-self.V_T = V_m? yes
    def beta_m_Na(self, v) -> np.longdouble: return ((+0.28*(v-self.V_T-40)) / (exp(-(v-self.V_T-40)/5)-1))
    def calc_m_Na(self, v, mpre, dt) -> np.float64: 
        dx = self.alpha_m_Na(v)*(1-mpre) - self.beta_m_Na(v)*mpre
        return forwardEuler(dx, mpre, dt)
    ...
    # K ---------------------------------------------------------------------
    # m
    def alpha_m_K(self, v)-> np.longdouble: return (-0.032*(v-self.V_T-15)) / (-1-exp(-0.02*(v-self.V_T-15)))
    def beta_m_K(self, v)-> np.longdouble: return 0.5*exp(-(v-self.V_T-10)/40)
    def calc_m_K(self, v, mpre, dt)-> np.float64:
        dx = self.alpha_m_K(v)*(1-mpre) - self.beta_m_K(v)*mpre
        return forwardEuler(dx, mpre, dt)
    ...
    # L ---------------------------------------------------------------------
    # high threshold Ca current (Reuveni). This one should be ICa in Vardalakis
    # m (q)
    def alpha_m_L(self, v)-> np.longdouble: return 0.055*(-27-v) / (exp((-27-v)/17) - 1) # qua pero rimane solo v e non vm intesa come v-vt
    def beta_m_L(self, v)-> np.longdouble: return 0.94*exp((-75-v)/17)
    def calc_m_L(self, v, mpre, dt)-> np.float64: 
        dx = self.alpha_m_L(v)*(1-mpre) - self.beta_m_L(v)*mpre
        return forwardEuler(dx, mpre, dt)
...
class Interneuron:
    V_T         = -30       # (mV) adjust spike threshold used to be -55
    V_X         = 2         # (mV)
    TAU_MAX     = 1e3       # (ms)
